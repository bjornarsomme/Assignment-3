...
public class LinKernighan {
     ...
     */
    public LinKernighan(ArrayList<Vertex1> vertex1s, ArrayList<Integer> ids) {
        this.ids = ids;
    ...
    public int[] constructNewTour(int[] tour, ArrayList<Integer> changes) {
        ArrayList<Edge1> currentEdges = deriveEdgesFromTour(tour);

        ArrayList<Edge1> X = deriveX(changes);
        ArrayList<Edge1> Y = deriveY(changes);
        int s = currentEdges.size();

        // Remove Xs
        for(Edge1 e: X) {
            for(int j = 0; j < currentEdges.size(); ++j) {
                Edge1 m = currentEdges.get(j);
                if(e.equals(m)) {
                    s--;
     ...
     */
    private int[] createTourFromEdges(ArrayList<Edge1> currentEdges, int s) {
        int[] tour = new int[s];
     ...
     */
    public ArrayList<Edge1> deriveX(ArrayList<Integer> changes) {
        ArrayList<Edge1> es = new ArrayList<Edge1>();
        for(int i = 1; i < changes.size() - 2; i+=2) {
            Edge1 e = new Edge1(tour[changes.get(i)], tour[changes.get(i+1)]);
            es.add(e);
        }
        return es;
    }
     ...
     */
    ArrayList<Edge1> deriveY(ArrayList<Integer> changes) {
        ArrayList<Edge1> es = new ArrayList<Edge1>();
     ...
     */
    public ArrayList<Edge1> deriveEdgesFromTour(int[] tour) {
        ArrayList<Edge1> es = new ArrayList<Edge1>();
        for(int i = 0; i < tour.length ; ++i) {
            Edge1 e = new Edge1(tour[i], tour[(i+1)%tour.length]);
            es.add(e);
        }
