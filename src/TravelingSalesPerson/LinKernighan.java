...
public class LinKernighan {
    ...
    // The distance table
    private Dictionary<String, Vertex>[] distances;
     ...
     */
    public void improve(int city1, boolean previous) {
        int city2 = previous? getPreviousIdx(city1): getNextIdx(city1);
        int city3 = getNearestNeighbor(city2);

        if(city3 != -1 && getDistance(city2, city3) < getDistance(city1, city2)) { // Implementing the gain criteria
            startAlgorithm(city1,city2,city3);
        } else if(!previous) {
            improve(city1, true);
        }
    }
